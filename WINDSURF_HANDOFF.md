...
- **Project:** FitFusion
- **Type:** Android mobile application — fitness game with AR overlay
...
- Default `lib/main.dart` (Flutter counter app — not yet replaced)
...
```
lib/
  [ ] main.dart                         — replace default counter app
  [ ] app.dart                          — MaterialApp, theme, routes
  [ ] firebase_options.dart             — already exists, do not touch
  [ ] core/
      [ ] constants.dart
      [ ] enums.dart
      [ ] theme.dart
  ...
  [ ] features/
      [ ] motion/
          ...
          [ ] pace_monitor.dart
      [ ] screens/
          [ ] splash_screen.dart        — minimal stub, just routes to home
          [ ] home_screen.dart          — minimal stub, just a Play button
          [ ] game_screen.dart          — camera + pose overlay, no Flame yet
  [ ] widgets/
...
→ The byte format conversion is the most common failure point. Verify `InputImageFormat.yuv420` matches the format reported by `CameraImage.format.group`. On some Android devices this is `ImageFormatGroup.yuv420`, on others it may differ. Add a log statement to print `image.format.group` and verify.

**"Rep detection fires too easily / phantom reps"**
→ Rolling average window is too small, or threshold is too permissive. Increase window size from 5 to 7 frames. Tighten the threshold constant. Test with deliberate slow movements to find the boundary.

**"Rep detection doesn't fire at all / misses real reps"**
→ Threshold is too strict. Check landmark likelihood — if you're filtering too aggressively, most frames get discarded. Lower `kLandmarkLikelihoodThreshold` from 0.5 to 0.4 as a test.

**"Camera preview is black or frozen"**
→ `hardwareAccelerated="true"` missing from AndroidManifest. Or `CameraController` was not initialized before `CameraPreview` widget was built. Use `FutureBuilder` to wait for `_cameraController.initialize()`.

**"Flame game covers the camera feed"**
→ `FitFusionGame` background is not transparent. Add `backgroundColor: Colors.transparent` to the `FlameGame` constructor or override `backgroundColor()`.
...
→ SHA-1 fingerprint is not registered in Firebase Console, or the wrong fingerprint was added. Run `cd android && ./gradlew signingReport` and copy the debug SHA-1. Register it at Firebase Console → Project Settings → Your Apps → Android App → Add Fingerprint.
